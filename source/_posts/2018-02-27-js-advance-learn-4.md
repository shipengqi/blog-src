...
离开了 DOM，JavaScript 就无法控制网页。后面介绍的就是 JavaScript 对 DOM 标准的实现和用法。
## DOM
### 节点
...
> 这里**注意**`Attribute`，在前端开发中`Property`(属性) 和 `Attribute`(特性)非常容易混淆，实际上，二者是不同的东西。
> `Property`是DOM中的属性，是JavaScript里的对象；
> `Attribute`是HTML标签上的特性，它的值只能够是字符串；
> 还有常用的`Attribute`，例如`id`、`class`、`title`等，会被作为`Property`附加到DOM对象上，可以和`Property`一样取值和赋值。
> 但是自定义的`Attribute`是不会直接放到DOM对象上的。


### 节点树

一个文档的所有节点，按照所在的层级，可以抽象成一种树状结构。这种树状结构就是 DOM 树。
浏览器原生提供顶层`document`节点，代表整个文档。
...
上面代码中，通过`node.parentNode`属性将`node`节点从文档里面移除，也就是删除自己。
